...
def main(door_count, no_door_count, org_img, width=320, height=240, debug=True):
    SSR.serial_setServo(20, 500, 500)
    time.sleep(1)

    print('进入start_door')
    print(door_count, no_door_count)
    while True:
        t1 = cv2.getTickCount()
        ...
        if percent > 20:
            ...
            door_count += 1
            no_door_count = 0
            return door_count, no_door_count
            ...
            if no_door_count >= 10:
                print('gogogo')
                print(percent)
                SSR.change_action_value("seu_fastgo", 8)  # gogogo
                time.sleep(6)
                return door_count, no_door_count
        else:
            return door_count, no_door_count
